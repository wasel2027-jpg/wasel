...
## تحديثات: البحث الذكي واقتراحات الصداقة
- **البحث الذكي (عربي ⇄ إنجليزي):** دلوقتي البحث في `search-results.html` بيطابق الاسم حتى لو كلمة البحث بلغة والاسم المخزّن بلغة تانية (زي "احمد" تلاقي "Ahmed")، عن طريق تحويل كل كلمة لبصمة صوتية ومقارنتها. ده تطابق تقريبي (heuristic) مش مثالي 100% في كل الأسماء، بس بيغطي غالبية الأسماء الشائعة.
- **حقول المحافظة/الدولة/المدينة:** اتضافت لمودال "تعديل البيانات الشخصية" في `profile.html` (`country`, `governorate`, `city` في `users/{uid}`) — اختيارية، وبتُستخدم في اقتراحات الصداقة.
- **اقتراحات الصداقة:** تبويب جديد "اقتراحات" في `friend-requests.html`، وكمان ويدجت "أشخاص تعرفهم" في `home.html`. الترتيب: عدد الأصدقاء المشتركين من الأعلى للأقل، وبعدين نفس المدينة/المحافظة/الدولة. الحسابات كلها بتتم بقراءات متوافقة مع الـ Rules الحالية (من غير أي تعديل مطلوب على Rules).
